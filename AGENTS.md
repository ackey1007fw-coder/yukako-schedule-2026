...
- 優花子さんの写真・配信スクリーンショット・切り抜き動画・サムネイル等を新たに掲載／転載するときは、**実際に使う素材をオーナー「あっきー」から本人へ事前確認し、明確なOKを得たものだけ掲載する**。サイト、X、Instagram、TikTok、YouTube、画像ZIPなどへの展開を対象とする。
- 確認するのは素材そのものと掲載先・公開範囲。オーナーからの「各SNSに展開して」という制作依頼、過去の別素材への許可、元配信・本人投稿が公開済みという事実、AI側の「かわいい」という判断は、今回の素材への本人承認の代わりにならない。本人の承認を得たこととその対象範囲は、オーナーの明示確認で受け取る。
- **未確認・返信待ち・承認範囲不明は非公開の下書きで止める**。未承認素材を公開リポジトリ／PR、公開プレビュー、`public/`、公開共有URLへ先に置かない。限定公開も承認不要とはみなさず、許可の範囲を確認する。
- 写り・表情・見せ方の可否は本人の判断を優先する。承認後に別カット・別サムネイルへ差し替えたり、意味や見え方が変わる編集をしたり、承認外の掲載先へ展開したりするときは再確認する。顔のAI生成・加工で承認を迂回しない。
- 投稿・予約投稿・サイト反映の直前に、対象素材と掲載先が承認範囲内であり、取り下げられていないことを再確認する。既存の制作依頼や投稿待ちの素材、再試行・再投稿にも適用する。CI成功や自律マージの許可は本人承認を代替しない。
- 本人から取り下げ希望が出た素材は再利用・再投稿を止め、掲載先と派生物を確認して削除／非公開対応を優先する。掲載先ごとの実確認が終わるまで「削除済み」と報告しない。録画原本の廃棄とは区別する。
- 後続エージェントへの非公開の引き継ぎには、素材の識別子、承認状態、掲載先・公開範囲、確認日を残す。私的な会話・スクリーンショット・承認メッセージそのものは、この公開リポジトリやPRへ載せない。
...
- マージ前に必ず確認するチェックリスト（1つでも欠けたらマージしない）:
  ...
  6. 写真・動画を新たに掲載／転載する変更では、対象素材・掲載先・公開範囲への本人承認を確認済み。未確認・取り下げ済みの素材を含まない。
...
- ❌「#ゆかJET 歌ダンス稽古動画が公開。LIVEでの楽曲披露の予告も。」
...
- `tailwind.config.ts` の colors トークン:
  ...
  - `champagne` #c29a4a（ゴールド＝装飾・淡い背景上のラベル/境界）
