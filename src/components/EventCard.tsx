  ...
  return (
    <article className={`yukako-ticket-card yukako-card yukako-card-interactive group relative grid overflow-hidden bg-white ${event.isImportant || isNext ? "border-champagne/70" : "border-rosefog/25"} ${compact ? "sm:grid-cols-[112px_1fr]" : "sm:grid-cols-[132px_1fr]"}`}>
      <div className="yukako-ticket-date relative flex items-center gap-4 border-b border-dashed border-champagne/45 bg-ink px-5 py-4 text-white sm:block sm:border-b-0 sm:border-r sm:px-4 sm:py-5">
        ...
        <span className="text-xs font-bold text-white/70 sm:mt-2 sm:block">{ticketDate.weekday}</span>
        <span className="ml-auto grid h-10 w-10 shrink-0 place-items-center border border-champagne/45 bg-white/10 text-champagne sm:ml-0 sm:mt-6"><Icon className="h-5 w-5" aria-hidden="true" /></span>
      </div>
      <div className={`${compact ? "p-5" : "p-6"} flex min-w-0 flex-col`}>
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0"><h3 className={`font-display leading-tight text-ink ${compact ? "text-xl" : "text-2xl"}`}>{event.title}</h3>
            <div className="mt-3 grid gap-2 text-sm text-ink/60"><p className="flex gap-2"><CalendarDays className="mt-0.5 h-4 w-4 shrink-0 text-champagne" aria-hidden="true" /><span>{event.displayDate}</span></p>
        ...
        {upcoming && <details className="group mt-5 border-t border-rosefog/15 pt-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <a href={googleCalendarUrl(event)} target="_blank" rel="noopener noreferrer" onClick={() => trackPortalEvent("calendar_add", { event_id: event.id })} className="inline-flex min-h-10 items-center border border-rosefog/25 bg-porcelain px-3 py-2 text-xs font-bold text-ink/70"><CalendarPlus className="mr-1.5 h-4 w-4 text-champagne" aria-hidden="true" />カレンダー追加</a>
            <a href={xShareUrl(shareText, `${SITE_URL}#event-${event.id}`, shareContent)} target="_blank" rel="noopener noreferrer" onClick={() => trackPortalEvent("x_share", { event_id: event.id })} className="inline-flex min-h-10 items-center border border-rosefog/25 bg-porcelain px-3 py-2 text-xs font-bold text-ink/70"><Share2 className="mr-1.5 h-4 w-4 text-champagne" aria-hidden="true" />Xで共有</a>
          </div>
        </details>}
      </div>
    </article>
