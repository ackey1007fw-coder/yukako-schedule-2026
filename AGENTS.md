...
- **AIエージェントは自律的にマージしない**。ただし、オーナー「あっきー」が現在の会話で対象PRを特定できる形で「マージして」「問題なければマージして」などと明確に指示した場合は、下記の条件をすべて確認したうえでAIがマージしてよい。
  1. 対象リポジトリ・PR番号・base/headが正しい。
  2. PRがDraftではなく、競合がなく、GitHub上でマージ可能。
  3. 必須CI・guard・ビルド・テストがすべて成功。
  4. 未解決のレビュー指摘や重大な懸念がない。
  5. 指示はその対象PRの1回限り。過去の包括的な許可や曖昧な相づちから推測しない。
...
可能ならローカル（`pnpm dev`）か Vercel のプレビューで見た目を確認し、**PR として提出してオーナーの承認を待つ**。
オーナーから対象PRについて明確なマージ指示があった場合のみ、上記の安全条件を確認してAIがマージしてよい。
